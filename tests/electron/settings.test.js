...
beforeEach(async () => {
  ...
  vi.resetModules();
  ({ createSettings, buildDefaults } = await import("../../electron/settings.js"));
});
...
describe("settings store", () => {
  ...
  it("returns defaults for fresh install (no migration needed)", () => {
    ...
    expect(s.get("telegram.token")).toBe(defaults.telegram.token);
  ...
  it("migrates v1 store through all migrations including v6 workspace conversion", () => {
    ...
    expect(s2.get("youtube.apiKey")).toBe(defaults.youtube.apiKey);
    expect(s2.get("download.maxConcurrent")).toBe(3);
    expect(s2.get("telegram.groupId")).toBe(defaults.telegram.groupId);
