...
> **For agentic workers:** This plan has NO commit steps per the user's standing instruction "khÃ´ng auto-commit". Run TDD cycles (failing test â†’ implementation â†’ tests pass) but leave the working tree as an unstaged diff. The user commits manually after review.

**Goal:** Add `thumbAvatar` task module + refactor Settings screen into tabs.

**Architecture:** Task module follows existing `runX(config) â†’ { ok, outputs, errors }` pattern. Sharp does in-Node image compositing (no FFmpeg needed for this task). Settings UI splits into a thin `tabs.js` component + 9 small render functions; schema bumps v4â†’v5 with additive migration. No new IPC handlers, no new npm deps.
...
**Modify:**
- `electron/settings.js` â€” bump SCHEMA_VERSION 4â†’5, add `avatar` block to defaults, add `ui.lastSettingsTab`, append `if (v < 5)` migration branch
- `electron/ipc/queue.js` â€” register `thumbAvatar` runner
- `electron/renderer/components/sidebar.js` â€” add 7th nav item
- `electron/renderer/main.js` â€” register `renderThumbAvatar` screen + label
- `electron/renderer/screens/settings.js` â€” refactor body into tabs (9 sections); add Avatar tab
- `tests/fixtures/generate.js` â€” add `tiny-avatar-square.png` (red) + `tiny-avatar-square.jpg` (blue), both 100Ã—100

**Conventions:**
- No `git commit` step per standing rule.
- TDD: write failing test â†’ run to confirm fail â†’ implement â†’ run to confirm pass.
...
```js
// 5. tiny-avatar-square.png â€” 100Ã—100 solid red (avatar fixture)
...
console.log("âœ“ tiny-avatar-square.png");

// 6. tiny-avatar-square.jpg â€” 100Ã—100 solid blue (avatar fixture, JPG variant)
...
console.log("âœ“ tiny-avatar-square.jpg");
```
...
Expected output: existing 4 fixtures + the 2 new ones, all printing `âœ“`.
...
## Task 2: `_lib/circleCrop.js` â€” sharp helper
...
describe("circleCrop", () => {
  it("returns an 80Ã—80 PNG buffer", async () => {
...
/**
 * Resize an image to a square `size` Ã— `size`, then mask to a circle (alpha 0 outside).
...
## Task 3: `src/thumbAvatar.js` â€” task module
...
describe("runThumbAvatar", () => {
  it("produces n Ã— m output files in m folders, named by thumbnail", async () => {
  ...
  it("places avatar at top-left when position=top-left", async () => {
    ...
    // At (16+40, 16+40) â€” centre of avatar â€” should NOT be pure white (because avatar is red).
  ...
  it("skips pairs where avatar would not fit, records error, continues other pairs", async () => {
    ...
    const output = path.join(tmpDir, "out");
    fs.mkdirSync(thumbDir); fs.mkdirSync(avatarDir);
    // First thumb is too small (50Ã—50 cannot fit 80+2*16 = 112)
...
export async function runThumbAvatar(config) {
  ...
  if (!thumbDir || !fs.existsSync(thumbDir)) throw new Error("Folder thumbnail khÃ´ng tá»“n táº¡i.");
  if (!avatarDir || !fs.existsSync(avatarDir)) throw new Error("Folder avatar khÃ´ng tá»“n táº¡i.");
  if (!output) throw new Error("Thiáº¿u folder output.");
  if (!POSITIONS.has(position)) throw new Error(`Vá»‹ trÃ­ khÃ´ng há»£p lá»‡: ${position}`);

  const thumbs = fs.readdirSync(thumbDir).filter((n) => /\.jpe?g$/i.test(n)).sort();
  if (thumbs.length === 0) throw new Error("Folder khÃ´ng cÃ³ file thumbnail (.jpg).");

  const avatars = fs.readdirSync(avatarDir).filter((n) => /\.(jpe?g|png)$/i.test(n)).sort();
  if (avatars.length === 0) throw new Error("Folder khÃ´ng cÃ³ file avatar (.jpg/.png).");
  ...
  for (let j = 0; j < m; j++) {
    ...
    try {
      ...
      errors.push({ avatar: avatarName, message: `Lá»—i xá»­ lÃ½ avatar: ${err.message}` });
      ...
      try {
        ...
        if (size + 2 * margin > Math.min(W, H)) {
          errors.push({
            ...
            message: `Avatar quÃ¡ lá»›n so vá»›i thumbnail (${W}Ã—${H}, cáº§n â‰¥ ${size + 2 * margin}px)`,
          });
          runner.log("warn", `Skip ${thumbName} Ã— ${avatarName}: too small`);
          continue;
        }
        ...
        errors.push({
          thumb: thumbName, avatar: avatarName,
          message: `Lá»—i xá»­ lÃ½ áº£nh: ${err.message}`,
        });
        runner.log("error", `${thumbName} Ã— ${avatarName} failed: ${err.message}`);
      }
  ...
  runner.setProgress(100, `ÄÃ£ ghi ${outputs.length}/${n * m}`);
...
Expected: PASS all 8 tests (some may take 5â€“10 sec each due to real sharp work).

---

## Task 4: Settings v5 â€” schema migration
...
## Task 5: `tabs.js` â€” reusable tab strip component
...
export function mountTabs(mountEl, tabs, opts = {}) {
  ...
  function activate(id) {
    ...
    Promise.resolve(tab.render(bodyEl)).catch((err) => {
      bodyEl.innerHTML = `<div style="color:red">Lá»—i render tab: ${escape(err.message)}</div>`;
    });
    opts.onChange?.(id);
  }
...
(No tests â€” DOM-only component, covered by manual smoke + the settings refactor in Task 6.)
...
This is the largest single change. The existing single-page implementation (~265 lines) is split into 9 small render functions wired through `mountTabs`. Each render function uses the same `window.api.settings.set(...)` calls â€” no behaviour changes.
...
- The Workspace tab includes both Workspace path + Identifier (per spec Â§5.5).
...
export async function renderSettings(el) {
  ...
  el.innerHTML = `
    <div class="screen-header">âš™ï¸ CÃ i Ä‘áº·t</div>
  ...
  el.innerHTML = `
    <div class="field">
      ...
      <div class="field-row">
        ...
        <button id="ws-pick">ðŸ“‚ Äá»•iâ€¦</button>
      </div>
      <div class="help">NÆ¡i chá»©a cÃ¡c thÆ° má»¥c input/output máº·c Ä‘á»‹nh.</div>
    </div>
    <div class="field">
      <label>ðŸ·ï¸ MÃ£ Ä‘á»‹nh danh</label>
      <input id="identifier" type="text" value="${escapeAttr(s.tracking?.identifier ?? "")}">
      <div class="help">Hiá»‡n trong tin nháº¯n Telegram Ä‘á»ƒ phÃ¢n biá»‡t mÃ¡y / channel. Äá»ƒ trá»‘ng = fallback theo tÃªn workspace.</div>
    </div>
  `;
...
function renderFfmpegTab(el, s) {
  el.innerHTML = `
    ...
    <div class="field">
      <label>Sá»‘ luá»“ng tá»‘i Ä‘a</label>
...
function renderRenderTab(el, s) {
  el.innerHTML = `
    <div class="field"><label><input id="useGPU" type="checkbox" ${s.render.useGPU ? "checked" : ""}> DÃ¹ng GPU máº·c Ä‘á»‹nh</label></div>
    <div class="field"><label>ChromaKey color</label><input id="chromaColor" type="text" value="${escapeAttr(s.render.chromaKey.color)}"></div>
    <div class="field"><label>Opacity (0â€“1)</label><input id="opacity" type="number" step="0.05" min="0" max="1" value="${s.render.opacity}"></div>
...
function renderDownloadTab(el, s) {
  ...
  async function refreshYtdlpStatus() {
    ...
    const txt = st.exists
      ? `ÄÃ£ cÃ i (version ${st.version ?? "?"}, cáº­p nháº­t ${st.lastModified ? new Date(st.lastModified).toLocaleString("vi-VN") : "?"})`
  ...
  el.innerHTML = `
    <div class="field">
      ...
      <div class="field-row">
        ...
        <button type="button" id="tg-token-show">ðŸ‘</button>
      </div>
    </div>
    ...
    <div class="help">Nháº¥n Ctrl+Q Ä‘á»ƒ hiá»‡n/áº©n Group ID vÃ  Tracking chat ID.</div>
  `;
...
function renderAvatarTab(el, s) {
  el.innerHTML = `
    ...
    <div class="field">
      <label>Margin tá»« mÃ©p (px)</label>
      <input id="av-margin" type="number" min="0" max="256" value="${s.avatar?.margin ?? 16}">
      <div class="help">Khoáº£ng cÃ¡ch tá»« avatar Ä‘áº¿n mÃ©p thumbnail (chá»‰ Ã¡p dá»¥ng vá»›i 4 gÃ³c, khÃ´ng Ã¡p dá»¥ng vá»‹ trÃ­ giá»¯a).</div>
    </div>
  `;
...
function renderAboutTab(el, s, version, rerender) {
  el.innerHTML = `
    ...
    <button id="reset" class="danger">Reset settings vá» máº·c Ä‘á»‹nh (giá»¯ workspace + Ä‘á»‹nh danh)</button>
    <button id="reset-all" class="danger" style="margin-left:8px">ðŸ—‘ XoÃ¡ toÃ n bá»™ dá»¯ liá»‡u â€” lÃ m láº¡i tá»« Ä‘áº§u</button>
    <div class="help" style="margin-top:6px">"XoÃ¡ toÃ n bá»™" sáº½ xoÃ¡ settings + workspace + Ä‘á»‹nh danh + lastConfig. App sáº½ reload vÃ  yÃªu cáº§u onboarding láº¡i. Files trong folder workspace KHÃ”NG bá»‹ xoÃ¡.</div>
  `;
  el.querySelector("#reset").addEventListener("click", async () => {
    if (!confirm("Reset toÃ n bá»™ cÃ i Ä‘áº·t vá» máº·c Ä‘á»‹nh? (Workspace path sáº½ giá»¯ nguyÃªn)")) return;
    ...
    await window.api.settings.set({
      ...
      telegram: {
        ...
        trackingChatId: REDACTED_CHAT_ID,
      },
      avatar: { size: 80, margin: 16, lastPosition: "bottom-right" },
  ...
  el.querySelector("#reset-all").addEventListener("click", async () => {
    const confirmed = confirm(
      "Báº¡n cháº¯c cháº¯n muá»‘n XOÃ TOÃ€N Bá»˜ Dá»® LIá»†U?\n\n" +
      "- Settings, workspace path, Ä‘á»‹nh danh, lastConfig sáº½ bá»‹ xoÃ¡.\n" +
      "- App sáº½ reload vÃ  yÃªu cáº§u onboarding láº¡i.\n" +
      "- Files trong folder workspace KHÃ”NG bá»‹ xoÃ¡.\n\n" +
...
## Task 7: Sidebar â€” add 7th nav item
...
```js
    { id: "thumbAvatar", icon: "ðŸ˜‚", label: "Gáº¯n avatar vÃ o thumbnail" },
```

---

## Task 8: `screens/thumbAvatar.js` â€” task form
...
const POSITIONS = [
  { id: "top-left",     label: "TrÃªn trÃ¡i" },
  { id: "top-right",    label: "TrÃªn pháº£i" },
  { id: "bottom-left",  label: "DÆ°á»›i trÃ¡i" },
  { id: "bottom-right", label: "DÆ°á»›i pháº£i" },
  { id: "center",       label: "Giá»¯a" },
];
...
export async function renderThumbAvatar(el) {
  ...
  el.innerHTML = `
    <div class="screen-header">ðŸ˜‚ Gáº¯n avatar vÃ o thumbnail</div>
    <p class="screen-subtitle">Má»—i avatar sinh ra 1 folder chá»©a cÃ¡c thumbnail Ä‘Ã£ chÃ¨n avatar Ä‘Ã³.</p>
    <form id="task-form">
      <div class="field">
        <label>ðŸ“ Folder thumbnail (.jpg)</label>
        <div class="field-row">
          <input id="thumb-dir" type="text" value="${escapeAttr(lastThumb)}" required>
          <button type="button" id="pick-thumb">ðŸ“‚ Chá»nâ€¦</button>
        </div>
      </div>
      <div class="field">
        <label>ðŸ“ Folder avatar (.jpg/.png)</label>
        <div class="field-row">
          <input id="avatar-dir" type="text" value="${escapeAttr(lastAvatar)}" required>
          <button type="button" id="pick-avatar">ðŸ“‚ Chá»nâ€¦</button>
        </div>
      </div>
      <div class="field">
      ...
      <div class="field">
        <label>ðŸ“ Vá»‹ trÃ­ avatar</label>
        <div style="display:flex;flex-direction:column;gap:4px;margin-top:4px">
          ${POSITIONS.map((p) => `
            <label><input type="radio" name="position" value="${p.id}" ${p.id === lastPosition ? "checked" : ""}> ${p.label}</label>
          `).join("")}
        </div>
        <div class="help">Avatar size: <strong>${size}Ã—${size} px</strong> (Ä‘á»•i trong Settings â†’ Avatar)</div>
      </div>
...
Run through the spec Â§10 checklist:
- Settings opens on last-selected tab; tab switching works.
- Avatar tab persists size/margin changes across restart.
- thumbAvatar task with 2 thumbs Ã— 3 avatars â†’ 6 outputs in 3 folders.
- All 5 positions visually correct on a 1280Ã—720 thumbnail.
...
1. **Spec coverage**:
   - Â§3 task flow â†’ Tasks 2 (circleCrop) + 3 (thumbAvatar)
   - Â§4 form + sidebar â†’ Tasks 7 + 8
   - Â§5 tabs refactor â†’ Tasks 5 + 6
   - Â§6 schema migration â†’ Task 4
   - Â§7 IPC (no new handlers) â†’ Task 9 only adds runner registration
   - Â§8 errors â†’ covered in module (Task 3)
   - Â§9 tests â†’ Tasks 1 + 2 + 3 + 4
   - Â§10 manual smoke â†’ Task 10

2. **Placeholder scan**: no TBD/TODO/"add appropriate".

3. **Type consistency**: `runThumbAvatar` returns `{ ok, outputs, errors }` â€” matches QueueManager. `circleCrop(input, size)` returns `Buffer` â€” matches caller in thumbAvatar.
