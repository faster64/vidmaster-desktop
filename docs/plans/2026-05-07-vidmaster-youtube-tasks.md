...
**Architecture:** Each new task ships as a `src/<task>.js` module with the existing `runX(config) â†’ { ok, outputs, errors }` signature. A shared `_lib/youtube.js` (REST + Node `fetch`) and `_lib/ytdlp.js` (binary lifecycle) sit alongside. New IPC handlers expose binary status, file listing, and URL/info file reading. New renderer screens follow the existing `taskFormShell` pattern except `concat.js`, which uses a custom 3-stage form with a reorderable list component. Settings v1 â†’ v2 migration adds `youtube.*` and `download.*` keys with defaults.
...
**Modify:**
- `electron/settings.js` â€” add v2 migration + new defaults
- `electron/ipc/fs.js` â€” add listMp4, readUrlsFile, readVideoInfos handlers
- `electron/ipc/queue.js` â€” register `getUrls`, `download`, `concat` runners
- `electron/main.js` â€” register `registerYtdlpIpc`, auto-update hook
- `electron/preload.mjs` â€” expose `window.api.ytdlp`, extend `window.api.fs`
- `electron/renderer/components/sidebar.js` â€” add 3 nav items
- `electron/renderer/components/progressBar.js` â€” support sub-status text
- `electron/renderer/main.js` â€” register 3 new screens, label map
- `electron/renderer/screens/settings.js` â€” add YouTube + Download/yt-dlp groups
- `tests/fixtures/generate.js` â€” add audio/silent fixtures

---

## Task 1: Settings v2 â€” schema + migration
...
```js
  it("migrates v1 store to v2 with default youtube/download keys", () => {
    ...
    expect(s2.get("youtube.apiKey")).toBe("REDACTED_GOOGLE_API_KEY");
...
const DEFAULT_API_KEY = "REDACTED_GOOGLE_API_KEY";
...
## Task 2: `_lib/sanitize.js` â€” Windows-safe filename helper
...
describe("sanitizeFilename", () => {
  ...
  it("replaces unicode ellipsis and fullwidth question mark with space", () => {
    expect(sanitizeFilename("titleâ€¦end")).toBe("title end");
    expect(sanitizeFilename("questionï¼Ÿmark")).toBe("question mark");
  });
  ...
  it("normalises NFC", () => {
    // 'Ã©' as combining sequence vs precomposed
    const combining = "eÌ";
    expect(sanitizeFilename(combining)).toBe("Ã©");
  });

  it("preserves valid unicode", () => {
    expect(sanitizeFilename("Tiáº¿ng Viá»‡t")).toBe("Tiáº¿ng Viá»‡t");
  });
});
```
...
## Task 3: `_lib/youtube.js` â€” REST client
...
describe("fetchUploadsPlaylistId", () => {
  ...
  it("throws on empty items", async () => {
    ...
    await expect(fetchUploadsPlaylistId({ apiKey: "K", handle: "@nope" }))
      .rejects.toThrow(/KhÃ´ng tÃ¬m tháº¥y channel/);
  });
...
export async function fetchUploadsPlaylistId({ apiKey, handle, signal }) {
  ...
  if (!data.items || data.items.length === 0) {
    throw new Error(`KhÃ´ng tÃ¬m tháº¥y channel cho handle: ${h}`);
  }
...
## Task 4: `_lib/ytdlp.js` â€” binary manager
...
## Task 5: `src/getUrls.js` â€” task module
...
describe("runGetUrls", () => {
  ...
  it("emits progress 5% start â†’ 90% after fetch â†’ 100% after write", async () => {
    const yt = await import("../src/_lib/youtube.js");
    yt.fetchUploadsPlaylistId.mockResolvedValue("UU");
    yt.listAllUploadVideos.mockImplementation(async ({ onPage }) => {
...
export async function runGetUrls(config) {
  ...
  if (!handle) throw new Error("Thiáº¿u handle.");
  if (!apiKey) throw new Error("Thiáº¿u YouTube API key.");
  if (!workspace) throw new Error("Thiáº¿u workspace.");

  runner.checkAborted();
  runner.setProgress(5, "Äang tÃ¬m kÃªnh...");
  ...
  const videos = await listAllUploadVideos({
    ...
    onPage: ({ pagesDone, totalPages }) => {
      ...
      runner.setProgress(pct, `ÄÃ£ Ä‘á»c ${pagesDone}/${totalPages} trang`);
    },
  });

  runner.checkAborted();
  runner.setProgress(90, "Lá»c & sáº¯p xáº¿p...");
  ...
  runner.setProgress(100, `ÄÃ£ ghi ${sorted.length} URLs`);
...
## Task 6: `src/download.js` â€” task module
...
export async function runDownload(config) {
  ...
  if (!urlsFile) throw new Error("Thiáº¿u file URLs.");
  if (!output) throw new Error("Thiáº¿u folder output.");
  if (!ytdlpPath) throw new Error("Thiáº¿u Ä‘Æ°á»ng dáº«n yt-dlp.");

  runner.checkAborted();
  fs.mkdirSync(output, { recursive: true });

  // Phase 1: bootstrap yt-dlp (0â€“5%)
  if (!fs.existsSync(ytdlpPath)) {
    runner.log("info", "Äang táº£i yt-dlp.exe...");
    await ensureBinary({
      targetPath: ytdlpPath, signal,
      onProgress: (p) => runner.setProgress(p * 0.05, "Táº£i yt-dlp..."),
    });
  }
  ...
  const inflight = new Map(); // url â†’ percent string
  ...
  const refreshProgress = () => {
    const lines = [...inflight.entries()]
      .map(([u, p]) => `  â€¢ ${shortUrl(u)} ${p}%`)
      .join("\n");
    const overall = 5 + (completed / total) * 95;
    runner.setProgress(overall, lines ? `Äang táº£i:\n${lines}` : "");
  };
  ...
  runner.setProgress(100, `ÄÃ£ táº£i ${outputs.length}/${total}`);
...
Expected: `tests/fixtures/tiny-with-audio.mp4` and `tests/fixtures/tiny-silent.mp4` exist (~50â€“200 KB each).
...
## Task 8: `src/concat.js` â€” task module
...
describe("runConcat", () => {
  ...
  it("rejects on missing input", async () => {
    ...
    })).rejects.toThrow(/khÃ´ng tá»“n táº¡i/i);
  });
...
export async function runConcat(config) {
  ...
  if (!Array.isArray(inputs) || inputs.length < 2) {
    throw new Error("Cáº§n Ã­t nháº¥t 2 video Ä‘á»ƒ ná»‘i.");
  }
  for (const p of inputs) {
    if (!fs.existsSync(p)) throw new Error(`File khÃ´ng tá»“n táº¡i: ${p}`);
  }
  fs.mkdirSync(path.dirname(output), { recursive: true });

  runner.checkAborted();
  runner.setProgress(0, "PhÃ¢n tÃ­ch input...");
 ...
 * @param {(items)=>void} onChange  â€” fires after every reorder/check toggle.
  ...
  function render() {
    mountEl.innerHTML = items.map((it, i) => `
      <div class="rl-row" data-key="${it.key}" draggable="true" style="display:flex;align-items:center;gap:8px;padding:6px 4px;border-bottom:1px solid #eee;cursor:move">
        <span class="rl-handle" style="user-select:none;color:#888">â‹®â‹®</span>
...
## Task 15: Sidebar â€” add 3 new nav items
...
const NAV_ITEMS = [
  { group: "Tasks", items: [
    { id: "render",   icon: "ðŸŽ¬", label: "Render Video" },
    { id: "snow",     icon: "â„ï¸", label: "Táº¡o video tá»« áº£nh" },
    { id: "trim",     icon: "âœ‚ï¸", label: "Cáº¯t video 30s" },
    { id: "cutBg",    icon: "ðŸŽžï¸", label: "Chia nhá» video ná»n" },
    { id: "getUrls",  icon: "ðŸ”—", label: "Láº¥y link kÃªnh" },
    { id: "download", icon: "â¬‡ï¸", label: "Táº£i video" },
    { id: "concat",   icon: "ðŸª¡", label: "Ná»‘i video" },
  ]},
  { group: "Há»‡ thá»‘ng", items: [
    { id: "queue",    icon: "ðŸ“‹", label: "HÃ ng Ä‘á»£i" },
...
## Task 16: `screens/getUrls.js` â€” form + viewer
...
export async function renderGetUrls(el) {
  ...
  el.innerHTML = `
    <div class="screen-header">ðŸ”— Láº¥y link kÃªnh</div>
    <p class="screen-subtitle">Láº¥y danh sÃ¡ch video tá»« má»™t YouTube channel theo handle.</p>
    <form id="task-form">
      <div class="field" data-path="handle" data-kind="text">
        ...
        <div class="help">Báº¯t buá»™c. Báº¯t Ä‘áº§u báº±ng @ hoáº·c tÃªn handle thuáº§n.</div>
      </div>
      <div class="field">
        <div class="help">â“˜ Min duration: <strong>${s.youtube.minDurationMinutes}</strong> phÃºt Â· Sort: <strong>${s.youtube.sortOrder}</strong> (Ä‘á»•i trong Settings â†’ YouTube)</div>
      </div>
      <button type="submit" class="primary">â–¶  Thá»±c hiá»‡n</button>
    </form>
  ...
  el.querySelector("#task-form").addEventListener("submit", async (e) => {
    ...
    toast({ kind: "success", message: "âœ… ÄÃ£ thÃªm vÃ o hÃ ng Ä‘á»£i" });
  });
  ...
  window.api.queue.onUpdate(async (state) => {
    ...
    viewer.innerHTML = `
      <h3>Káº¿t quáº£ (${infos.length} video)</h3>
      <div style="display:flex;gap:8px;margin-bottom:8px">
        <button id="vw-open-folder">ðŸ“‚ Má»Ÿ folder</button>
        <button id="vw-open-urls">ðŸ“„ Má»Ÿ file urls.txt</button>
      </div>
...
export async function renderDownload(el) {
  ...
  el.innerHTML = `
    <div class="screen-header">â¬‡ï¸ Táº£i video</div>
    <p class="screen-subtitle">Äá»c file URLs (.txt) vÃ  táº£i video báº±ng yt-dlp.</p>
    <form id="task-form">
      <div class="field">
        <label>ðŸ“„ File URLs (.txt)</label>
        <div class="field-row">
          <input id="urls-file" type="text" name="urlsFile" value="${escape(lastUrlsFile)}" required>
          <button type="button" id="pick-urls">ðŸ“‚ Chá»nâ€¦</button>
        </div>
        <div class="help">Máº·c Ä‘á»‹nh: file Get URLs gáº§n nháº¥t.</div>
      </div>
      <div class="field">
        <label>ðŸ“ Folder output</label>
        <div class="field-row">
          <input id="output" type="text" name="output" value="${escape(lastOutput)}" required>
          <button type="button" id="pick-output">ðŸ“‚ Chá»nâ€¦</button>
        </div>
      </div>
      <div class="field">
        <label>ðŸ”¢ Sá»‘ táº£i song song <span id="cc-val">${lastConcurrent}</span></label>
        <input id="concurrent" type="range" min="1" max="5" value="${lastConcurrent}">
        <div class="help">Default tá»« Settings Â· range 1â€“5.</div>
      </div>
      <button type="submit" class="primary">â–¶  Thá»±c hiá»‡n</button>
    </form>
  `;
...
## Task 18: `screens/concat.js` â€” 3-stage form
...
export async function renderConcat(el) {
  ...
  el.innerHTML = `
    <div class="screen-header">ðŸª¡ Ná»‘i video</div>
    <p class="screen-subtitle">Ná»‘i cÃ¡c .mp4 trong má»™t folder thÃ nh má»™t video duy nháº¥t, giá»¯ nguyÃªn audio.</p>
    <form id="task-form">
      <div class="field">
        <label>ðŸ“ Folder input</label>
        <div class="field-row">
          <input id="folder" type="text" value="${escape(lastFolder)}" required>
          <button type="button" id="pick-folder">ðŸ“‚ Chá»nâ€¦</button>
          <button type="button" id="reload">â†»</button>
        </div>
      </div>

      <div class="field">
        <label>Danh sÃ¡ch video <span id="count">(0/0 chá»n)</span></label>
        <div id="rl" style="border:1px solid #ccc;border-radius:4px;max-height:360px;overflow:auto"></div>
        <div class="help">KÃ©o â‹®â‹® Ä‘á»ƒ Ä‘á»•i thá»© tá»±. Bá» check Ä‘á»ƒ loáº¡i trá»«.</div>
      </div>

      <div class="field">
        <label>ðŸ“ TÃªn file output</label>
        <div class="field-row">
      ...
      <button type="submit" class="primary" id="submit" disabled>â–¶  Thá»±c hiá»‡n</button>
    </form>
  `;
  ...
  function refreshOutputPreview() {
    ...
    outputPreview.textContent = `â†’ ${folder}\\${name}.mp4`;
  }

  function refreshSubmitState() {
    const items = rl?.getItems() ?? [];
    const checked = items.filter((i) => i.checked).length;
    countLabel.textContent = `(${checked}/${items.length} chá»n)`;
  ...
  async function reload() {
    ...
    rl = mountReorderableList(rlMount, files.map((f) => ({
      key: f.fullPath,
      label: f.name,
      meta: `${(f.sizeBytes / (1024*1024)).toFixed(1)} MB Â· ${formatDur(f.durationSec)}`,
      checked: true,
    })), refreshSubmitState);
    refreshSubmitState();
  ...
  el.querySelector("#task-form").addEventListener("submit", async (e) => {
    ...
    if (exists) {
      if (!confirm(`File ${output} Ä‘Ã£ tá»“n táº¡i. Ghi Ä‘Ã¨?`)) return;
    }
...
const TASK_LABELS = {
  ...
  getUrls: "Láº¥y link kÃªnh", download: "Táº£i video", concat: "Ná»‘i video",
};
```
...
## Task 20: Settings UI â€” YouTube + Download/yt-dlp groups
...
```html
    ...
    <div class="field">
      ...
      <div class="field-row">
        ...
        <button type="button" id="yt-key-show">ðŸ‘</button>
      </div>
    </div>
    <div class="field">
      <label>Min duration (phÃºt)</label>
    ...
    <div class="field">
      ...
      <div class="field-row">
        ...
        <button type="button" id="yt-path-pick">ðŸ“‚ Äá»•iâ€¦</button>
      </div>
    </div>
    <div class="field"><label><input id="yt-auto" type="checkbox" ${s.download.autoUpdateYtDlp ? "checked" : ""}> Tá»± cáº­p nháº­t yt-dlp khi má»Ÿ app</label></div>
    <div class="field">
    ...
    <div class="field">
      ...
      <div id="yt-status" class="help">Äang kiá»ƒm tra...</div>
      <button type="button" id="yt-update">â¬‡ï¸ Cáº­p nháº­t ngay</button>
    </div>
```
...
```js
  ...
  async function refreshYtdlpStatus() {
    ...
    const txt = st.exists
      ? `ÄÃ£ cÃ i (version ${st.version ?? "?"}, cáº­p nháº­t ${st.lastModified?.toString().slice(0, 24) ?? "?"})`
      : `ChÆ°a cÃ i táº¡i ${st.path}`;
...
Click each sidebar item in turn. Expect no console errors. The forms render. yt-dlp status in Settings reads "ChÆ°a cÃ i".

- [ ] **Step 3: Get URLs end-to-end**

In **Láº¥y link kÃªnh**, enter `@MrBeast` (or any real handle). Submit. Wait. Expect:
- Queue progress bar advances 5% â†’ 90% â†’ 100%.
- `<workspace>/channels/MrBeast/only_video_urls.txt` exists.
- Viewer table appears with rows.

- [ ] **Step 4: Download end-to-end**

In **Táº£i video**, file = the path from Step 3, output = `<workspace>/downloads/MrBeast`, concurrent = 1. Submit. Expect:
- Progress 0â€“5% during yt-dlp install (binary appears at the configured path).
- Progress climbs to 100%.
- `.mp4` and `.jpg` files appear in output folder, with sanitized filenames.

- [ ] **Step 5: Concat end-to-end**

In **Ná»‘i video**, folder = the download output. Verify the list shows mp4s with size + duration. Uncheck one, drag-reorder others. Submit. Expect output `.mp4` plays with audio.
...
Settings â†’ enable "Tá»± cáº­p nháº­t yt-dlp khi má»Ÿ app". Restart app. Expect log line `yt-dlp.exe updated on startup` in `%APPDATA%\VidMaster\logs\main.log`.
...
Expected: PASS â€” including pre-existing tests for render/snow/trim/cutBg.
...
1. Spec coverage: Each spec section maps to one or more tasks above.
   - Â§1 Overview â†’ Tasks 1, 5, 6, 8
   - Â§2 File layout â†’ Task structure
   - Â§3 Settings â†’ Task 1 (schema), Task 20 (UI)
   - Â§4 yt-dlp manager â†’ Task 4 (lib), Task 10 (IPC), Task 11 (auto-update hook)
   - Â§5 Get URLs â†’ Task 5 (module), Task 16 (screen)
   - Â§6 Download â†’ Task 6 (module), Task 17 (screen)
   - Â§7 Concat â†’ Task 8 (module), Task 14 (component), Task 18 (screen)
   - Â§8 IPC â†’ Tasks 9, 10, 12
   - Â§9 Errors â†’ covered inline (each module throws Vietnamese-summary errors; modal already exists)
   - Â§10 Testing â†’ Tasks 2, 3, 4, 5, 6, 7, 8, 21
   - Â§11 Dependencies â†’ no new npm deps; verified.
   - Â§12 Out of scope â†’ no tasks needed.
2. Placeholder scan: no TBD/TODO/"add appropriate" present.
3. Type consistency: `runGetUrls` returns `{ ok, outputs, videos, errors }`; `runDownload`, `runConcat` return `{ ok, outputs, errors }` â€” all match QueueManager's expectations.
